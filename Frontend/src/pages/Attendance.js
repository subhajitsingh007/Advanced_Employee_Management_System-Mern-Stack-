...
const Attendance = () => {
  ...
  const fetchAttendanceByEmployee = async (id) => {
    ...
    try {
      ...
      setSingleAttendance(response.data.attendance);
    } catch (err) {
    ...
    <div style={styles.page}>
      ...
      <div style={styles.card}>
        ...
        {loading ? (
          ...
          <div>
            <h3>{singleAttendance.employee.name}</h3>
            <p>Status: {singleAttendance.status}</p>
            <ul>
              {singleAttendance.attendanceLog.map((log, i) => (
                <li key={i}>
                  {log.logStatus} - {new Date(log.logDate).toLocaleDateString()}
                  {log.checkIn && <> | In: {new Date(log.checkIn).toLocaleTimeString()}</>}
                  {log.checkOut && <> | Out: {new Date(log.checkOut).toLocaleTimeString()}</>}
                </li>
              ))}
            </ul>
          </div>
            ...
            <table style={styles.table}>
              <thead>
                <tr>
                  ...
                  <th style={styles.th}>Log History</th>
                </tr>
              </thead>
              <tbody>
                {attendanceRecords.map((record) => (
                  <tr key={record._id} style={styles.tr}>
                    ...
                    <td style={styles.td}>{record.employee.email}</td>
                    <td style={styles.td}>{record.status}</td>
                    <td style={styles.td}>
                      <ul>
                        {record.attendanceLog.map((log, index) => (
                          <li key={index}>
                            {log.logStatus} - {new Date(log.logDate).toLocaleDateString()}
                            {log.checkIn && <span> - Checked In: {new Date(log.checkIn).toLocaleTimeString()}</span>}
                            {log.checkOut && <span> - Checked Out: {new Date(log.checkOut).toLocaleTimeString()}</span>}
                          </li>
                        ))}
                      </ul>
                    </td>
